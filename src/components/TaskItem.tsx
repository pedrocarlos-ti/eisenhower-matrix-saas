...
const TaskItem: React.FC<TaskItemProps> = ({ task, onEdit, onDelete, onToggleComplete }) => {
  ...
  return (
    <div
      className={`bg-white/90 backdrop-blur-sm p-4 rounded-xl border border-white/50 cursor-move hover:bg-white hover:shadow-md transition-all duration-200 group ${task.completed ? 'opacity-60' : ''}`}
      draggable
    ...
    >
      ...
      <div className="flex items-start space-x-3 mb-3">
        ...
        <div className="flex items-center space-x-2">
          {/* Priority indicator */}
          <div className={`w-2 h-2 rounded-full ${getPriorityColor(task.priority)}`} title={`${task.priority} priority`}></div>
